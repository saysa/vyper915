...
';
		...
		if(((isset($value['protected'][0]["id"]) && $value['protected'][0]["id"]!="") && $protected_value==$value['protected'][0]["value"]) || (!isset($value['protected'][0]["id"]))) {
			...
			echo $this->before_item.$this->before_item_title.$value['title'].$this->after_item_title.'
					<ul class="blocks block-available clearfix inactive-blocks" id="available-homepage-blocks">';
						...
						echo '
						<li class="inactive-block component" rel="'.esc_attr($block['type']).'" title="'.esc_attr($block['title']).'">
							<div class="blocks-content clearfix" title="'.esc_attr($block['title']).'">
								<a href="javascript:{}" class="button move" title="'.$block['title'].'">Move</a>';
								if(isset($block['image'])) {
									echo '<img src="'.esc_attr(THEME_IMAGE_CPANEL_URL.$block['image']).'" alt="'.esc_attr($block['title']).'" class="feature-img">';
								}
									...
									echo '
										<li class="active-block component dropped" rel="'.$block['type'].'" id="recordsArray_'.$block['id'].'" style="opacity: 1; z-index: 0; ">
											<div class="blocks-content clearfix">
												...
												if(isset($image)) {
													echo '<img src="'.THEME_IMAGE_CPANEL_URL.$image.'" alt="'.esc_attr($optTitle).'" class="feature-img">';
												}
									echo 	'<div></div>';
	...
	*/
	function print_meta_block($value){
		echo '<input type="hidden" name="sticky_meta_box_nonce" value="', wp_create_nonce("different-themes"), '" />';
			...
			<span>
				<div class="export">
					...
					<script type="text/javascript">
						jQuery(document).ready(function($){ loadUploader(jQuery("span#<?php echo $value['id'];?>_button"), document.URL+'&df-export=upload&df-export-type=<?php echo $value['section'];?>');});
					</script>
				</div>
			</span>
		</div>
	...
	*/
	function print_meta_sub_navigation($value, $i){
		...
		if($value['subname']){
			echo '<div class="dfpost-tabbed-wrapper meta_sub_tabs">';
			echo '<div class="tabs clearfix">';
			echo '<ul class="dfpost-tabbed-menu">';
			foreach($value['subname'] as $subname ){
	...
	function print_meta_sub_tab($value, $i){
		...
		if((isset($value['skip_templates']) && df_template_check($value['skip_templates'])==false) || !isset($value['skip_templates'])) { //meta box check
			if(isset($value['hide_in']) && (in_array(df_get_current_post_type(),$value['hide_in']))) {
				...
				echo '<div id="sub_tabs-'.($i-1).'-'.$value['slug'].'" class="dfpost-tabbed-blocks">';
			}
		}
	}	
		...
		<ul class="blocks block-active clearfix" id="sidebar_order">
			...
			foreach ( $saved_value as $sidebar ) {
				if ( $sidebar != "" ) {
					$i++;			
			?>
					<li class="row-item-full-width clearfix" id="recordsArray_<?php echo convert_to_class($sidebar); ?>" alt="<?php echo esc_attr($sidebar); ?>">
	...
	*/
	function print_add_text($value) {
		...
		<a href="javascript:{}" class="button-1 df-save-management" data-saved="<?php _e("Add/Save", THEME_NAME);?>" data-loading="<?php _e("Adding...", THEME_NAME);?>"><?php _e("Add/Save", THEME_NAME);?></a>
			...
			<?php } ?>
				...
				<span class="select">
					<select name="<?php echo $value["id"];?>" class="styled">
						...
						<option value="DFoff" <?php if($input_value=="DFoff") { echo "selected=\"selected\""; } ?>><?php _e("Off", THEME_NAME);?></option>
	...
	*/
	...
	?>
		<div class="row">
			<input type="hidden" name="action" value="save" /><a href="javascript:{}" class="button-1 df-save-management" data-saved="<?php echo $value['title'];?>" data-loading="<?php _e("Saving...", THEME_NAME);?>"><?php echo $value['title'];?></a>
		</div>
	<?php
	}
			...
			<ul class="blocks block-active slider-sequence clearfix">
				...
				?>
					<li class="row-item-full-width clearfix" id="recordsArray_<?php echo $post->ID; ?>">
						<div class="blocks-content clearfix">
							<div class="image"><img src="<?php echo $image['src']; ?>" alt="<?php echo esc_attr(get_the_title()); ?>" /></div>
