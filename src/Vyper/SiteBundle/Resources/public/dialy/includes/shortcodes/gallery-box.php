	...
	if(isset($atts['url'])) {
		...
		if(is_object($page)) {
			...
			if(is_numeric($id)) {
				...
				$galleryImages = get_post_meta ( $id, "_".THEME_NAME."_gallery_images", true ); 
				...
				$content.=	'<div class="gallery-preview">';
					$content.=	'<div class="photos">';
	            		...
	            		foreach($imageIDs as $imgID) { 
	            			...
	            			if($imgID) {
								...
								$content.=	'<div class="img-block">
												<a href="'.$atts['url'].'?page='.$counter.'" class="hover-effect" data-id="gallery-'.$id.'">
													<img src="'.esc_attr($image['src']).'" alt="'.esc_attr($page->post_title).'" title="'.esc_attr($page->post_title).'" data-id="'.$counter.'"/>
												</a>
											</div>';
						...
						$content.=	'<a href="'.$atts['url'].'" class="img-block" data-id="gallery-'.$id.'">
					...
					$content.=	'</div>';
					$content.=	'<h2><a href="'.$atts['url'].'" data-id="gallery-'.$id.'">'.$page->post_title.'</a></h2>';
